...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, padding: '8px 12px', fontSize: 12 }}>
      ...
      <div style={{ color: val < 0 ? '#B94A34' : '#8261A8', fontWeight: 600 }}>{formatKRW(val)}</div>
    </div>
  );
};
...
export default function AllowancePanel({ transactions, year, month, stats }) {
  ...
  const barColor = usedPct > 90 ? '#B94A34' : usedPct > 70 ? '#B58BD0' : '#8261A8';
  ...
  return (
    <div className="allowance-panel" style={{ alignSelf: 'start' }}>
      ...
      <div className="allowance-nums">
        ...
        <span style={{ color: isOverBudget ? '#B94A34' : undefined, fontWeight: isOverBudget ? 700 : undefined }}>
          {isOverBudget
      ...
      <ResponsiveContainer width="100%" height={120}>
        <LineChart data={allowanceData} margin={{ top: 4, right: 8, left: -30, bottom: 0 }}>
          ...
          <Line type="monotone" dataKey="balance" stroke="#8261A8" strokeWidth={2} dot={false} activeDot={{ r: 4, fill: '#8261A8' }} />
        </LineChart>
      </ResponsiveContainer>
